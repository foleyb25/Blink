...
class Animations {
    ...
    func animateRecordButton(videoButton: UIButton, captureButton: UIButton) {
    ...
    private func moveAndExpand(fromButton: UIButton, toButton: UIButton) {
    ...
    func animateMoveRecordButtonBack(button: UIButton) {
    ...
    func animatePulsatingLayer(layer: CALayer) {
    ...
    private func pulsateRecordButton(button: UIButton) {
        let animation = CABasicAnimation(keyPath: "backgroundColor")
        animation.fromValue = UIColor.clear.cgColor
        animation.toValue = UIColor(red: 1, green: 0, blue: 0, alpha: 0.5).cgColor
        animation.duration = 0.75
    ...
    private func startRecordTimer(button: UIButton) {
        timer.invalidate()
        seconds = 0
        button.setTitle("\(seconds)", for: .normal)
        timer = Timer.scheduledTimer(withTimeInterval: 1, repeats: true) { (Timer) in
            self.seconds+=1
            button.setTitle("\(self.seconds)", for: .normal)
        }
